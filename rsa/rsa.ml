...
let decrypt str p q public_key =
...
let step_3 p q =
   "Now define the totient of the modulus, i.e
...
let step_4 public_key =
   "We need a public key, or in other words, a large
  prime exponent that we can use to encrypt our message. Let's
  choose k = " ^ public_key

(*[step_5] explains how to generate the private key*)
let step_5 public_key p q =
   "The private key is generated by the recepient, defined
  as the inverse of k with respect to mod (totient). We can compute this
  by using the extended Euclidean algorithm. This turns out to be"
...
let step_6 p q public_key =
   "So now we have the following definitions:
    ...
    "let private_key =" ^ (private_key p q public_key)

(*[step_7 msg] encrypts the user's message, and displays the resulting
 *encryption*)
let step_7 msg =
  let encrpyted_msg = encrypt msg in
 "The encrypted message then is calculated by
  computing [msg]**(public_key) mod n. The result is the following: " ^
  (list_to_string encrpyted_msg)

(*[step_8 lst] decrypts the message to a large int displayed on the
 *screen. *)
let step_8 lst p q public_key =
  let decrypted_lst = decrypt lst p q public_key in
