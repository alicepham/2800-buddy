...
let main () =
  let window = GWindow.window ~height: 600 ~width: 1000
  ...
  let regex_button_page = GButton.button ~label: "Go to regex Simplification!"
    ~packing:(hbox_turing_button#add) () in 
      regex_button_page#connect#clicked
      ~callback: (fun () -> (notebook#goto_page 3));
  ...
  let box_entry_num_input = GPack.hbox ~height: 20 ~width: 100 ~spacing: 10 
    ~homogeneous: true ~packing: vbox_turing#add  () in
  ...
  let box_make_matrix = GPack.hbox ~height: 20 ~width: 100 ~spacing: 10 
    ~homogeneous: true ~packing: vbox_turing#add  () in
  let box_trans_matrix = GPack.vbox ~border_width: 5 ~packing: vbox_turing#add () in
